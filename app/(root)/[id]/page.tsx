...
import { fetchBlogPostDetail, fetchBlogPosts } from '@/lib/api';
...
async function BlogDetails({
  ...
  const md = MarkdownIt();
  const blogs: BlogInterface[] = await fetchBlogPosts();
    ...
    <section className='py-3'>
      <div className='max-w-5xl mx-auto px-4 xl:px-0 space-y-4'>
        ...
        <div className='w-full h-[30rem] overflow-hidden rounded-lg border'>
          <img
            src={post?.cover_image !== null ? post?.cover_image : post?.social_image}
            alt='cover img'
